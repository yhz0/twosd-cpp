...
StageStochasticPattern StochasticPattern::filter_by_stage(int stage)
{
    int _stage = stage;
    size_t _rv_count = rv_count[stage];

    std::vector<int> row_index(_rv_count), col_index(_rv_count);
    std::vector<double> reference_values(_rv_count);
    std::vector<size_t> indices_in_scenario(_rv_count);

    size_t cnt = 0;
    for (size_t i = 0; i < this->stage.size(); ++i)
    {
        if (this->stage[i] == stage)
        {
            row_index[cnt] = this->row_index[i];
            col_index[cnt] = this->col_index[i];
            reference_values[cnt] = this->reference_values[i];
            indices_in_scenario[cnt] = i;
            cnt++;
        }
    }

    return StageStochasticPattern(_stage, row_index, col_index, reference_values, indices_in_scenario);
}

StageStochasticPattern::StageStochasticPattern(int _stage, const std::vector<int> &_row_index,
                                               const std::vector<int> &_col_index, const std::vector<double> &_reference_values,
                                               const std::vector<size_t> &_indices_in_scenario)
    : stage(_stage), row_index(_row_index), col_index(_col_index),
      reference_values(_reference_values), indices_in_scenario(_indices_in_scenario), rv_count(_indices_in_scenario.size()) {}

// Definitions for Iterator methods

StageStochasticPattern::Iterator::Iterator(const StageStochasticPattern &pattern, const std::vector<double> &global_scenario_vector, size_t pos)
    : pattern_(pattern), global_scenario_vector_(global_scenario_vector), pos_(pos) {}

bool StageStochasticPattern::Iterator::operator!=(const Iterator &other) const
{
    return pos_ != other.pos_;
}

const StageStochasticPattern::Iterator &StageStochasticPattern::Iterator::operator++()
{
    ++pos_;
    return *this;
}

StageStochasticPattern::Iterator::Element StageStochasticPattern::Iterator::operator*() const
{
    size_t global_index = pattern_.indices_in_scenario[pos_];
    double scenario_value = global_scenario_vector_[global_index];
    double delta_value = scenario_value - pattern_.reference_values[pos_];
    return Element(pattern_.row_index[pos_], pattern_.col_index[pos_], scenario_value, delta_value);
}

StageStochasticPattern::Iterator::Element::Element(int _row_index, int _col_index, double _scenario_value, double _delta_value)
    : row_index(_row_index), col_index(_col_index), scenario_value(_scenario_value), delta_value(_delta_value) {}

StageStochasticPattern::Iterator StageStochasticPattern::begin(const std::vector<double> &global_scenario_vector) const
{
    return Iterator(*this, global_scenario_vector);
}

StageStochasticPattern::Iterator StageStochasticPattern::end(const std::vector<double> &global_scenario_vector) const
{
    return Iterator(*this, global_scenario_vector, rv_count);
}